...
(function () {
    ...
    var ARCH = {
        ...
        height: 1630,
        flows: {
            creator:   { title: 'Creator Flow',        quote: '\u201CI need someone for my project.\u201D',         x: 76,  y: 24 },
            crew:      { title: 'Crew Flow',           quote: '\u201CI\u2019m looking for the right project.\u201D', x: 76,  y: 636 },
            shared:    { title: 'Shared Project',      quote: '\u201CWhere both experiences converge.\u201D',       x: 1060, y: 470 },
            workspace: { title: 'Project Workspace',   quote: '\u201CWhere the project gets built together.\u201D', x: 1270, y: 24 }
        },
        zone: { market: 'Marketplace' },
        nodes: [
            /* ---------- Creator flow (upper-left) ---------- */
            { id: 'c-onboard', label: 'Creator Onboarding', cat: 'creator', level: 1, x: 80, y: 110,
                desc: 'Entry point for Creators \u2014 account, identity and intent are captured up front.' },
            { id: 'c-profile', label: 'Creator Profile', cat: 'creator', level: 3, x: 80, y: 250,
                desc: 'The public Creator identity collaborators will see.' },
            { id: 'c-dashboard', label: 'Dashboard', cat: 'creator', level: 2, x: 80, y: 390,
                desc: 'Home base for managing projects, hiring and payments.' },
            { id: 'c-projects', label: 'Projects', cat: 'creator', level: 3, x: 80, y: 540,
                desc: 'Every posted brief, from draft to completed project.' },
            { id: 'c-discover', label: 'Discover Crew', cat: 'creator', level: 2, x: 300, y: 250,
                desc: 'Browse and filter Crew members by craft, availability and rating.' },
            { id: 'c-crewprofile', label: 'Crew Profile', cat: 'creator', level: 3, x: 300, y: 390,
                desc: 'Evaluate a Crew member before opening a conversation.' },
            { id: 'c-post', label: 'Post Project', cat: 'creator', level: 2, x: 520, y: 170,
                desc: 'Creator defines the brief, requirements, budget and timeline.' },
            { id: 'c-review', label: 'Review Project', cat: 'creator', level: 3, x: 520, y: 310,
                desc: 'Check the brief, scope and budget before publishing.' },
            { id: 'c-payment', label: 'Payment', cat: 'creator', level: 3, x: 520, y: 450,
                desc: 'Funding is set up before the project goes live.' },
            { id: 'c-published', label: 'Project Published', cat: 'creator', level: 2, x: 520, y: 590,
                desc: 'The brief goes live \u2014 Crew members can now view and apply.' },
            { id: 'c-apps', label: 'Applications', cat: 'creator', level: 2, x: 760, y: 350,
                desc: 'Creator reviews Crew members, evaluates portfolios and shortlists candidates.' },
            { id: 'c-shortlist', label: 'Shortlist', cat: 'creator', level: 3, x: 760, y: 490,
                desc: 'Compare the strongest candidates side by side.' },
            { id: 'c-hire', label: 'Hire', cat: 'creator', level: 2, x: 760, y: 630,
                desc: 'The conversation becomes a commitment \u2014 and a shared project.' },

            /* ---------- Crew flow (lower-left) ---------- */
            { id: 'k-onboard', label: 'Crew Onboarding', cat: 'crew', level: 1, x: 80, y: 740,
                desc: 'Entry point for Crew \u2014 portfolio, skills and availability.' },
            { id: 'k-profile', label: 'Crew Profile', cat: 'crew', level: 2, x: 80, y: 880,
                desc: 'The portfolio Creators evaluate when shortlisting.' },
            { id: 'k-dashboard', label: 'Dashboard', cat: 'crew', level: 2, x: 80, y: 1020,
                desc: 'Work in motion \u2014 applications, projects and earnings.' },
            { id: 'k-discover', label: 'Discover Projects', cat: 'crew', level: 2, x: 300, y: 840,
                desc: 'Browse open briefs that match craft, timeline and budget.' },
            { id: 'k-details', label: 'Project Details', cat: 'crew', level: 3, x: 300, y: 980,
                desc: 'Read the full brief and requirements before applying.' },
            { id: 'k-apply', label: 'Apply', cat: 'crew', level: 2, x: 520, y: 760,
                desc: 'Submit a tailored proposal against the brief.' },
            { id: 'k-status', label: 'Application Status', cat: 'crew', level: 3, x: 520, y: 900,
                desc: 'Follow the application \u2014 received, viewed or shortlisted.' },
            { id: 'k-shortlisted', label: 'Shortlisted', cat: 'crew', level: 2, x: 740, y: 640,
                desc: 'The Creator singled out your work for review.' },
            { id: 'k-hired', label: 'Hired', cat: 'crew', level: 2, x: 740, y: 780,
                desc: 'Accepting the work opens the shared project workspace.' },

            /* ---------- Shared project (centre-right) ---------- */
            { id: 's-project', label: 'Shared Project', cat: 'shared', level: 1, x: 1040, y: 590, stacked: true,
                sub: 'where both experiences converge',
                desc: 'Where both journeys converge \u2014 a single source of truth for communication, files, deliverables and revisions.' },

            /* ---------- Project workspace (far right) ---------- */
            { id: 'w-workspace', label: 'Project Workspace', cat: 'workspace', level: 1, x: 1330, y: 130,
                desc: 'Shared environment where Creator and Crew manage one project together.' },
            { id: 'w-overview', label: 'Overview', cat: 'workspace', level: 3, x: 1330, y: 270,
                desc: 'Project summary, status and next actions at a glance.' },
            { id: 'w-req', label: 'Requirements', cat: 'workspace', level: 3, x: 1330, y: 410,
                desc: 'Brief, scope and acceptance criteria, always accessible.' },
            { id: 'w-timeline', label: 'Timeline', cat: 'workspace', level: 3, x: 1330, y: 550,
                desc: 'Milestones, deadlines and dependencies.' },
            { id: 'w-chat', label: 'Chat', cat: 'workspace', level: 3, x: 1330, y: 690,
                desc: 'Day-to-day conversation between Creator and Crew.' },
            { id: 'w-files', label: 'Files', cat: 'workspace', level: 3, x: 1330, y: 830,
                desc: 'References, drafts and assets in one place.' },
            { id: 'w-del', label: 'Deliverables', cat: 'workspace', level: 2, x: 1330, y: 970,
                desc: 'Work gets submitted against the brief.' },
            { id: 'w-review', label: 'Review', cat: 'workspace', level: 2, x: 1330, y: 1110,
                desc: 'Creator evaluates the deliverable against requirements.' },
            { id: 'w-revise', label: 'Revision', cat: 'workspace', level: 3, x: 1330, y: 1250,
                desc: 'Iterate until the deliverable is approved.' },
            { id: 'w-approve', label: 'Approval', cat: 'workspace', level: 2, x: 1330, y: 1390,
                desc: 'The deliverable is accepted and the milestone closes.' },
            { id: 'w-complete', label: 'Payment / Completion', cat: 'workspace', level: 2, x: 1330, y: 1530,
                desc: 'Funds are released and the project is completed.' }
        ],
        edges: [
            ['c-onboard', 'c-profile'], ['c-profile', 'c-dashboard'],
            ['c-dashboard', 'c-discover'], ['c-dashboard', 'c-projects'],
            ['c-discover', 'c-crewprofile'], ['c-crewprofile', 'c-post'],
            ['c-post', 'c-review'], ['c-review', 'c-payment'], ['c-payment', 'c-published'],
            ['c-published', 'c-apps'], ['c-apps', 'c-shortlist'], ['c-shortlist', 'c-hire'],
            ['c-hire', 's-project'],

            ['k-onboard', 'k-profile'], ['k-profile', 'k-dashboard'],
            ['k-dashboard', 'k-discover'], ['k-discover', 'k-details'], ['k-details', 'k-apply'],
            ['k-apply', 'k-status'], ['k-status', 'k-shortlisted'], ['k-shortlisted', 'k-hired'],
            ['k-hired', 's-project'],
    ...
    function buildCanvas() {
        ...
        ARCH.nodes.forEach(function (n) {
            var node = el(n.stacked ? 'div' : 'div', 'fc-arch-node');
    ...
    function boot() {
        canvasEl = document.getElementById('fc-arch-canvas');
        if (!canvasEl) return;
